...
const AppComponent: React.FC = () => {
    ...
    useEffect(() => {
        if (isFirebaseConfigured) {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Initial data load timed out")), 8000));
    ...
    const handleResetDay = (date: string) => {
        if (window.confirm(`Are you sure you want to permanently reset all data for ${date}? This action cannot be undone.`) &&
            window.confirm(`FINAL CONFIRMATION: Delete all guest counts, sales, assignments, and attendance for ${date}?`)) {
                setDailyCounts(prev => { const next = {...prev}; delete next[date]; return next; });
                setPackageSalesData(prev => { const next = {...prev}; delete next[date]; return next; });
                setDailyAssignments(prev => { const next = {...prev}; delete next[date]; return next; });
                setTSAssignments(prev => { const next = {...prev}; delete next[date]; return next; });
                setAttendanceData(prev => { const next = {...prev}; delete next[date]; return next; });
                
                showNotification(`All data for ${date} has been reset.`, 'success');
                logAction('RESET_DAY', `All operational data for ${date} was permanently deleted.`);
           }
    };
